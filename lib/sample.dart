...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> signInUser() async {
    try {
      ...
      // Determine the user role based on user type
      String userRole = await getUserRole(userCredential.user!.uid);

      // Navigate to the appropriate dashboard based on the user's role
      if (userRole == 'Admin') {
        // Navigate to CorporateDashboard for admin
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) =>
                CorporateDashboard(user: userCredential.user!),
          ),
        );
      } else if (userRole == 'Blue' || userRole == 'Grey') {
        // Navigate to UserDashboard for regular users
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => UserDashboard(user: userCredential.user!),
          ),
        );
      } else {
        print('Unknown user role');
